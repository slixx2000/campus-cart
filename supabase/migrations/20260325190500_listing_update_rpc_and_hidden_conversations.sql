-- Ensure owner update policy checks both current and new row ownership.
...
-- RPC helper used by mobile app for listing status updates.
...
-- Per-user hidden conversations: deletes from inbox for one user without removing chat for the other participant.
